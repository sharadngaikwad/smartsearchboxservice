...
const rankResults = (results, question, userPreference, keyword, email) => {
    ...
    return cloudant.searchBySelector(config.cloudant.userHistoryDB, {email: email})
        ...
        .then(finalResults => {
            finalResults.forEach(f => f.score = convertScore(f.score)); //To convert the score to % TODO: Need to think of this to put it inside some loop
...
const convertScore = score => parseFloat(score/config.watsonDiscovery.maxScore * 100).toFixed(2) + '%';
