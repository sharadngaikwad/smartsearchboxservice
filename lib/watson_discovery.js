 ...
 * */
const watsonDiscoveryResponse = (payload, q, appName, email) => {
        ...
        discovery.query(payload, ((err, discoveryData) => {
            if (err) {
                ...
                if (discoveryData && discoveryData.results && discoveryData.results.length > 0) {
                    ...
                    return userHistory.setApplicationTrendForUser(email, appName)
                        .then(userPreference => rank.rankResults(outputArray, q, userPreference))
                        .then(rankedArray => {
                            resolve({
                                type: 'discovery',
