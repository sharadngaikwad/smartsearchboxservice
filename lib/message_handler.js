                        ...
                        if (snowQuery.priorityNo || snowQuery.incidentNo) {
                            if (snowQuery.priorityNo) {
                                if (snowQuery.issue) {
                                    ...
                                    snowUrlQuery = 'sysparm_query=' + snowQuery.priorityNo + '^active=true^state!=6^assigned_toLIKE' + userName;
                                }
                                if(snowQuery.state) {
                                    snowUrlQuery = snowUrlQuery + '^' + snowQuery.state;
                                } else {
                                    snowUrlQuery = snowUrlQuery + '^stateNOT IN6,3';
                                }
                            }
                            ...
                            snowUrlQuery = 'sysparm_query=active=true^state!=6^' + snowQuery.issue;
                            if(snowQuery.state) {
                                snowUrlQuery = snowUrlQuery + '^' + snowQuery.state;
                            } else {
                                snowUrlQuery = snowUrlQuery + '^stateNOT IN6,3';
                            ...
                            snowUrlQuery = 'sysparm_query=active=true^state!=6^assigned_toLIKE' + userName;
                            if(snowQuery.state) {
                                snowUrlQuery = snowUrlQuery + '^' + snowQuery.state;
                            } else {
                                snowUrlQuery = snowUrlQuery + '^stateNOT IN6,3';
...
const snowQueryGenerator = entity => {
    ...
    entity.forEach(e => {
        ...
        if (e.entity === 'incident-status') {
            state = 'state=' + config.snow.incidentStates[e.value];
        }
